...
import { ConnectedHomeProps } from './ConnectedHome';

type Props = ConnectedHomeProps & {
}

export const Home: React.FC<Props> = ({fetchGameTypes, gameTypes}) => {
    const history = useHistory();

    React.useEffect(fetchGameTypes, []);
    ...
    const startGameCards = gameTypes.map(gameType => {
        return {
            description: gameType.description,
            title: gameType.displayName,
            onClick: () => history.push("/new-game?type="+gameType.name)
        }
    });
    ...
    return (
        <div>
            ...
            <CardCarousel title="Start a Hard-coded Game" cards={hardCodedCards}/>
